...
'''
STUFF TO MAYBE TRY LATER
'''
'''               
#To flip an image - both work. 
image_flip4 = cv2.flip(image,1) # the 1 says vertical axis, 0 would be horizontal axis
image_flip = np.fliplr(image)

#try rotating image 04-1735_0.jpg
rows,cols = gray.shape
M = cv2.getRotationMatrix2D((cols/2,rows/2),45,1)
gray_tilt = cv2.warpAffine(gray,M,(cols,rows))
misc.imsave('image_flip4tilt.jpg', gray_tilt)
